...
def get_updates():
    ...
    try:
        method = 'getUpdates'
        r = session.get(f'https://api.telegram.org/bot{token}/{method}', timeout=5)
        ...
        print('Неудачный вызов GetUpdate')
#        print(e)
...
def send_messages(id_list, text):
    for i in id_list:
        ...
        try:
            ...
            syslog_to_csv(f'Неудачная отправка сообщения пользователю {i}')
            time.sleep(1)
...
def handler_updates(message):
    ...
    if text == '/start':
        t = '''Здравствуйте, я бот энергокомплекса на Павловском ДОКе. 
Я буду вас оповещать о событиях на комплексе, таких как запуск/остановка оборудования и переход в "островной" режим.
После отправки команды /start, вы были автоматически подписаны на рассылку бота, чтоб отписаться отправьте команду: /del_me.
Обратите внимание, все команды начинаются со знака: "/".
Подсказки о доступных командах, вы увидите ниже, нажав внизу знак "/" в квадрате.'''
        send_messages([message_id], t)

    if text == '/wtf':
        df = pd.read_csv(data_file, parse_dates=['Дата Время'], index_col=['Дата Время'])[-5:]
        t = f"*{datetime.datetime.now().strftime('%d.%m.%Y %H:%M:%S')}*\n\n*ГПГУ 1: *{int(df['ГПГУ 1 '].mean())} кВт\n*ГПГУ 2: *{int(df['ГПГУ 2 '].mean())} кВт\n*ГПГУ 3: *{int(df['ГПГУ 3 '].mean())} кВт\n*ГПГУ 4: *{int(df['ГПГУ 4 '].mean())} кВт\n*ГПГУ 5: *{int(df['ГПГУ 5 '].mean())} кВт\n\n*Мощность завода: *{int(df['Мощность завода'].mean())} кВт\n*Сумм мощность ГПГУ: *{int(df['Сумм мощность ГПГУ'].mean())} кВт\n*Импорт: *{int(df['MainsImport'].mean())} кВт\n\n*MWh: *{df['MWh'][-1]}"
        send_messages([message_id], t)
